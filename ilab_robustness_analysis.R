...
logisticRegression_performace <- function(training_data, testing_data,settings){
...
#Loading data ctu-13 dataset
#feature_vectors_cleaned <- read.csv('/home/jguerra/datasets/stratosphere/ctu_13_feature_vector_cleaned.txt', stringsAsFactors = F, sep = '|')
#feature_vectors_cleaned$class <- as.factor(feature_vectors_cleaned$class)
#feature_vectors_cleaned <- feature_vectors_cleaned %>% mutate(subclass = ifelse(grepl('Normal', class),'normal','botnet'))
#feature_vectors_cleaned$subclass <- as.factor(feature_vectors_cleaned$subclass)
...
balanced_accuracy <- logisticRegression_performace(training,testing,ctrl_fast)
total <- nrow(training)
result_rf_accuracy <- data.frame('count_of_data' = partitions)
result_ela_measure <- data.frame('count_of_data' = partitions)
write.table(result_rf_accuracy,file="/home/jguerra/datasets/stratosphere/balanced_ilab_test_result_rf_accuracy_2.txt",sep="|", row.names = F)
write.table(result_ela_measure,file="/home/jguerra/datasets/stratosphere/balanced_ilab_test_result_ela_measure_2.txt",sep="|", row.names = F)
for (j in c(1:30)){
  write(paste('ITERATION',j,sep=' '),file="/home/jguerra/datasets/stratosphere/test.txt",append = TRUE)
  ...
  for(i in partitions){
    ...
    balanced_accuracy_aux <- logisticRegression_performace(training_noisy, testing, ctrl_fast)
