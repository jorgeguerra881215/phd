...
##Ploting result
```{r}
myData_port_section <- myData %>% filter(port %in% port_data$port[1:11])
#ports <- c('22', '25', '53', '80', '123', '135', '443', '3128', '6667', '7600', '54728')

ports <- port_data$port[1:11]
aux_vector_port <- c(rep(ports, each = 2))
class_vector <- c(rep(c("Normal", "Botnet"), times = 11))
frequency_values <- c()
count <- 1
for(i in 1:length(ports)){
  aux_port <- ports[i]
  aux_vector <- port_data %>% filter(port == aux_port)
  frequency_values[count] = ifelse(aux_vector$coun_normal == 0, NaN, aux_vector$coun_normal) 
  frequency_values[count+1] = ifelse(aux_vector$coun_botnet == 0, NaN, aux_vector$coun_botnet)
  count = count + 2
}
mix_data <- data.frame('ports' = aux_vector_port, 'class' = class_vector, 'freq' = frequency_values)
aux_freq <- frequency_values
aux_freq[is.na(aux_freq)] <- 0

#library(plyr)
mix_data <- ddply(mix_data, .(ports), 
   transform, pos = cumsum(aux_freq) - (0.5 * aux_freq)
)

# plot bars and add text
mix_data$ports = as.factor(mix_data$ports)
p <- ggplot(mix_data, aes(x = ports, y = freq)) +
     ...
     scale_fill_manual("legend", values = c("Normal" = "#00bfc4ff", "Botnet" = "#f8766dff")) + 
     theme_bw()

     
p + scale_y_sqrt()
```
### Same graph whit different code
```{r}
...
## Analisys by port. Learning rate resulting from just one iteration(5% = 4000 connections)
```{r}
learning_rate_result <- read.csv('/home/jguerra/repo/result_learning_rate_rf_by_connection.txt',sep = '|', stringsAsFactors = F)
feature_vectors_cleaned <- read.csv('/home/jguerra/repo/feature_vector_cleaned.txt',sep = '|', stringsAsFactors = F)
...
ggplot(visual123, aes(x=x,y=m,  group=group, col=group, fill=group))+
  #geom_errorbar(aes(ymin=m-(ds/2), ymax=m+(ds/2)), colour="black", width=0.2) +
  geom_line() + 
  #geom_point() +
  geom_errorbar(aes(ymin=m-(ds/2), ymax=m+(ds/2)), colour="black", width=0.2) +
