...
#botnet_prob_result %>% filter(TrueClass == 'Botnet') %>% group_by(subclass) %>% summarise(n=n(),RF_mean=mean(RamdomForest),KNN_mean=mean(KNN),LR_mean=mean(LogisticRegression),NB_mean=mean(NaiveBayes),SVM_mean=mean(SVM)) %>% arrange(desc(n))
