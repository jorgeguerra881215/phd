...
a <- confusionMatrix(predsrf,testing$class)
a$overall
str(a$overall)
```
...
### Comparing Models
```{r}
resamps <- resamples(list(rf = rfFit,knn = knnFit, lr = logicRFit, nv = naiveBayesFit, svm = svmFit))
summary(resamps)
bwplot(resamps2)
diffs <- diff(resamps)
summary(diffs)
...
#write_csv(x = botnet_prob_result, 'conection_probabilistic_table.csv')
```
...
botnet_prob_result_table_subclass$subclass = as.factor(botnet_prob_result_table_subclass$subclass)
data <- cbind(botnet_prob_result_table_subclass,id=seq(1:nrow(botnet_prob_result_table_subclass)))
data <- reshape2::melt(data[,-1],id.vars=c("id"))
data <- cbind(sub_class = botnet_prob_result_table_subclass$subclass,data)
p01 <- ggplot(data, aes(sub_class, value)) +
  geom_bar(aes(fill = variable), position = "dodge", stat="identity")

botnet_prob_result_table_port$port = as.factor(botnet_prob_result_table_port$port)
data <- cbind(botnet_prob_result_table_port,id=seq(1:nrow(botnet_prob_result_table_port)))
data <- reshape2::melt(data[,-1],id.vars=c("id"))
data <- cbind(ports = botnet_prob_result_table_port$port,data)
p02 <- ggplot(data, aes(ports, value)) +
  geom_bar(aes(fill = variable), position = "dodge", stat="identity")

botnet_prob_result_table_proto$proto = as.factor(botnet_prob_result_table_proto$proto)
data <- cbind(botnet_prob_result_table_proto,id=seq(1:nrow(botnet_prob_result_table_proto)))
data<-reshape2::melt(data[,-1],id.vars=c("id"))
data = cbind(protos = botnet_prob_result_table_proto$proto,data)
p03 <- ggplot(data, aes(protos, value)) +
  geom_bar(aes(fill = variable), position = "dodge", stat="identity")
...
p3 <- ggplot(data, aes(protos, value)) +
  geom_bar(aes(fill = variable), position = "dodge", stat="identity")
multiplot(p02, p2, p03, p3,cols=2)
```
...
g3 <- ggplot(pca_testing_botnet, aes(PC1, PC2, col=as.factor(cluster))) + 
  geom_point(aes(shape=as.factor(cluster)), size=2) +   # draw points
  labs(title="Botnet Clustering", 
       subtitle="With principal components PC1 and PC2 as X and Y axis",
       caption="Source: Strathosphere") + 
...
set.seed(200)
#Bagging by port
c1 = training %>% filter(port == '80')
c2 = training %>% filter(port %in% c('25','443'))
c3 = training %>% filter(port %in% c('6667','53'))
c4 = training %>% filter(!(port %in% c('80','25','443','6667','53')))

#Bagging by cluster TODO

rfFit_by_port = train(class ~ sp+wp+wnp+snp+ds+dm+dl+ss+sm+sl,
               data = c1,
               metric="ROC",
               method = "rf",
               trControl = ctrl_fast)
rfFit_by_port$finalModel

rfFit_by_port_c2 = train(class ~ sp+wp+wnp+snp+ds+dm+dl+ss+sm+sl,
               data = c2,
               metric="ROC",
               method = "rf",
               trControl = ctrl_fast)
rfFit_by_port_c2$finalModel

rfFit_by_port_c3 = train(class ~ sp+wp+wnp+snp+ds+dm+dl+ss+sm+sl,
               data = c3,
               metric="ROC",
               method = "rf",
               trControl = ctrl_fast)
...
rfFit_by_port_c4 = train(class ~ sp+wp+wnp+snp+ds+dm+dl+ss+sm+sl,
               data = c4,
               metric="ROC",
               method = "rf",
               trControl = ctrl_fast)
rfFit_by_port_c4$finalModel
```
### Bagging using adabag
```{r}
library(adabag)
set.seed(2)
training.bagging = bagging(class ~ sp+wp+wnp+snp+ds+dm+dl+ss+sm+sl,data = training, mfinal = 10)
training.bagging$importance

training.predbagging = predict.bagging(training.bagging,newdata = testing)
training.predbagging$confusion
training.predbagging$error

training.bagging.cv = bagging.cv(class ~ sp+wp+wnp+snp+ds+dm+dl+ss+sm+sl,data = training,v = 10, mfinal = 10)
training.bagging.cv$confusion
training.bagging.cv$error

```
### Training with some data set size.
```{r}
set.seed(200)
library(doParallel)
cl <- makeCluster(2)
...
count_random <- foreach(i=3:13) %dopar% {
  ((i*(i+1))/2)^2
}
count_random
metric <- foreach(i=3:13) %dopar% {
  library(caret)
  count <- ((i*(i+1))/2)^2
  aux_training_set <- training[sample(size_training, count), ]
  
  #training Random Forest with new training set.
  ...
  predsrfprobs_2 <- predict(new_rfFit,testing,type='prob')
  predsrf_2 <- ifelse(predsrfprobs_2$Botnet >=0.5,'Botnet','Normal')
  cm <- confusionMatrix(predsrf_2,testing$class)
  metric <- cm$overall[1]
  metric
}
...
### Making some comparative graphic
```{r}
rfResult = predsrf
knnResult = ifelse(knnPredict$Botnet >=0.5,'Botnet','Normal')
logisticResult = ifelse(logicRPredict$Botnet >=0.5,'Botnet','Normal')
naiveResult = ifelse(naiveBayesPredict$Botnet >=0.5,'Botnet','Normal')
svmResult = ifelse(svmPredict$Botnet >=0.5,'Botnet','Normal')
testingResult = testing$class

testing

#PCA 
trans = caret::preProcess(testing[,1:10], 
                   method=c("BoxCox", "center", 
                            "scale", "pca"))
PC = predict(trans, testing[,1:10])


# log transform 
#log.ir <- log(testing[,1:10])
#ir.species <- iris[, 5]
 
# apply PCA - scale. = TRUE is highly 
# advisable, but default is FALSE. 
#ir.pca <- prcomp(testing[,1:10],
#                 center = TRUE,
#                 scale. = TRUE) 
```
### Graph of Random forest prediction result
```{r}
rfDataResult = data.frame(PC$PC1, PC$PC2, ifelse(rfResult == testingResult, 'CorrectlyPredicted', 'WronglyPredicted'), testing$port)
names(rfDataResult) = c('x', 'y', 'prediction', 'port')
rfDataResult$port = as.factor(rfDataResult$port)

rfDataResult_percent = rfDataResult %>% filter(!is.na(port)) %>% group_by(prediction, port) %>% dplyr::summarise(n = n()) %>% arrange(desc(n))

rfGraphResult <- ggplot(rfDataResult_percent,aes(x = port, y = n,fill = prediction)) + 
    ...
    scale_fill_manual("", values = c("CorrectlyPredicted" = "#00bfc4ff", "WronglyPredicted" = "#f8766dff")) + 
    theme_bw() +
    theme(axis.text = element_text(size = 12),
...
rfGraphResult <- ggplot(rfDataResult %>% filter(!is.na(port))) +
     geom_bar(aes(x = port, fill = prediction)) +
     #geom_text(aes(label = freq, y = pos), size = 3) + 
     labs(title="", 
       caption="Source: Testing Dataset", 
       x = "Port",
       y="RF Prediction Result", 
       color=NULL) +
     scale_fill_manual("", values = c("CorrectlyPredicted" = "#00bfc4ff", "WronglyPredicted" = "#f8766dff")) +
     theme_bw()+
     theme(axis.text = element_text(size = 12),
...
rfGraphResult + scale_y_continuous(trans = 'log2')
rfGraphResult

```

### Graph of K-NN prediction result
```{r}
knnDataResult = data.frame(PC$PC1, PC$PC2, ifelse(knnResult == testingResult, 'CorrectlyPredicted', 'WronglyPredicted'), testing$port)
names(knnDataResult) = c('x', 'y', 'prediction', 'port')
knnDataResult$port = as.factor(knnDataResult$port)

knnGraphResult <- ggplot(knnDataResult %>% filter(!is.na(port))) +
     geom_bar(aes(x = port, fill = prediction)) +
     #geom_text(aes(label = freq, y = pos), size = 3) +
     labs(title="", 
       caption="Source: Testing Dataset", 
       x = "Port",
       y="K-NN Prediction Result", 
       color=NULL) +
     scale_fill_manual("", values = c("CorrectlyPredicted" = "#00bfc4ff", "WronglyPredicted" = "#f8766dff")) +
     theme_bw()+
     theme(axis.text = element_text(size = 12),
...
knnGraphResult + scale_y_sqrt()

```

### Graph of Logistic Regression prediction result
```{r}
logisticDataResult = data.frame(PC$PC1, PC$PC2, ifelse(logisticResult == testingResult, 'CorrectlyPredicted', 'WronglyPredicted'), testing$port)
names(logisticDataResult) = c('x', 'y', 'prediction', 'port')
logisticDataResult$port = as.factor(logisticDataResult$port)

logisticGraphResult <- ggplot(logisticDataResult %>% filter(!is.na(port))) +
     geom_bar(aes(x = port, fill = prediction)) +
     #geom_text(aes(label = freq, y = pos), size = 3) +
     labs(title="", 
       caption="Source: Testing Dataset", 
       x = "Port",
       y="Logistic Regression Prediction Result", 
       color=NULL) +
     scale_fill_manual("", values = c("CorrectlyPredicted" = "#00bfc4ff", "WronglyPredicted" = "#f8766dff")) +
     theme_bw()+
     theme(axis.text = element_text(size = 12),
...
logisticGraphResult + scale_y_sqrt()

```

### Graph of Naive Bayes prediction result
```{r}
naiveDataResult = data.frame(PC$PC1, PC$PC2, ifelse(naiveResult == testingResult, 'CorrectlyPredicted', 'WronglyPredicted'), testing$port)
names(naiveDataResult) = c('x', 'y', 'prediction', 'port')
naiveDataResult$port = as.factor(naiveDataResult$port)

naiveGraphResult <- ggplot(naiveDataResult %>% filter(!is.na(port))) +
     geom_bar(aes(x = port, fill = prediction)) +
     #geom_text(aes(label = freq, y = pos), size = 3) +
     labs(title="", 
       caption="Source: Testing Dataset", 
       x = "Port",
       y="Naive Bayes Prediction Result", 
       color=NULL) +
     scale_fill_manual("", values = c("CorrectlyPredicted" = "#00bfc4ff", "WronglyPredicted" = "#f8766dff")) +
     theme_bw()+
     theme(axis.text = element_text(size = 12),
...
naiveGraphResult + scale_y_sqrt()

```

### Graph of Support Vector Machine prediction result
```{r}
svmDataResult = data.frame(PC$PC1, PC$PC2, ifelse(svmResult == testingResult, 'CorrectlyPredicted', 'WronglyPredicted'), testing$port)
names(svmDataResult) = c('x', 'y', 'prediction', 'port')
svmDataResult$port = as.factor(svmDataResult$port)

svmGraphResult <- ggplot(svmDataResult %>% filter(!is.na(port))) +
     geom_bar(aes(x = port, fill = prediction)) +
     #geom_text(aes(label = freq, y = pos), size = 3) +
     labs(title="", 
       caption="Source: Testing Dataset", 
       x = "Port",
       y="Support VM Prediction Result", 
       color=NULL) +
     scale_fill_manual("", values = c("CorrectlyPredicted" = "#00bfc4ff", "WronglyPredicted" = "#f8766dff")) +
     theme_bw()+
     theme(axis.text = element_text(size = 12),
